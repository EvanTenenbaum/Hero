...
export async function getSessionToken(): Promise<string | null> {
  try {
    ...
    if (Platform.OS === "web") {
      if (isDev) console.log("[Auth] Web platform uses cookie-based auth");
      return null;
    }
...
export async function setSessionToken(token: string): Promise<void> {
  try {
    ...
    if (isDev) console.log("[Auth] Session token stored successfully");
  } catch (error) {
...
export async function removeSessionToken(): Promise<void> {
  try {
    ...
    if (isDev) console.log("[Auth] Session token removed successfully");
  } catch (error) {
